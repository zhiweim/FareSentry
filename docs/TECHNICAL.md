# FareSentry Technical Reference

[← Project overview and public demo](../README.md)

This document preserves the implementation notes, detailed examples, and
operational caveats for the hackathon version of FareSentry.
For the product story and minimal setup, start with the
[README Quick Start](../README.md#quick-start--local-demo).

**Run all commands from the repository root, not from `docs/`.**
Code paths and example filenames below are relative to that root.
Live examples can incur charges; automated tests and the public demo use
synthetic inputs and do not invoke live services.

## Contents

- [Local demo behavior](#local-hackathon-demo)
- [Local Live Mode](#live-mode)
- [Community Cloud deployment and maintenance](#public-hackathon-deployment-streamlit-community-cloud)
- [Python environment setup](#local-setup-windows-powershell)
- [AWS login and Bedrock setup](#bedrock-local-setup-windows-powershell)
- [Checks](#checks) and [repository structure](#structure)
- [Models, normalization, and return lookup](#models-and-serpapi-normalization)
- [SerpApi setup and smoke test](#serpapi-local-setup-and-real-data-smoke-test)
- [Recommendation facts and structured validation](#recommendations)
- [SQLite schema, runs, and historical queries](#sqlite-fare-history)
- [Deterministic alert semantics](#deterministic-opportunity-decisions)
- [Monitoring cycle](#one-monitoring-cycle)
- [Local scheduling](#local-watch-scheduling)
- [Notification delivery](#email-notification-delivery)
- [SES configuration](#ses-configuration) and [transport smoke test](#explicit-manual-transport-smoke-test)
...
FareSentry is deployed on **Streamlit Community Cloud** at
**[faresentry.streamlit.app](https://faresentry.streamlit.app/)**, with
...
### Deployment settings and maintenance

The deployment uses repository **zhiweim/FareSentry**, branch **main**, entry point
**streamlit_app.py**, and **Python 3.13**. Keep **Secrets empty**, including
`FARESENTRY_ENABLE_LIVE`, and preserve public access for judges. Never commit
`.env`, AWS credentials, local databases, or `.streamlit/secrets.toml` (ignored).

For redeployment or a separate instance, use the same settings in
[Streamlit Community Cloud](https://share.streamlit.io/) and follow the
[platform deployment instructions](https://docs.streamlit.io/deploy/streamlit-community-cloud/deploy-your-app/deploy).
After an update, check access in a signed-out browser and rerun both scenarios:
**Completed + silent** at USD 1,000, then **Simulated notification delivered** at
USD 800. See [public sharing settings](https://docs.streamlit.io/deploy/streamlit-community-cloud/share-your-app).
...
operations remain deferred. The hosted page does not run a background worker,
...
requirements.txt              Hosted install of the existing ui extra
streamlit_app.py               Demo-first Streamlit UI and explicit local actions
.streamlit/config.toml         Streamlit presentation and server settings
LICENSE                       MIT License
...
src/faresentry/
    ...
    monitoring.py             One bounded monitoring cycle and completion semantics
    scheduling.py             Sequential due-watch checks and local polling loop
    notifications.py          Alert-gated formatting, delivery, and deduplication
    notification_models.py    Typed messages, outcomes, and successful receipts
    demo.py                   Synthetic scenarios using real application services
    demo_live.py              Optional local SerpApi/Bedrock/SES composition
    persistence/
    ...
    providers/
        ...
        ses.py                Lazy Amazon SES email transport
        _logging.py           SerpApi transport-secret protection
scripts/
    search_flights.py         Manual real-data smoke test
    send_test_email.py        Explicit manual SES transport smoke test
tests/
    conftest.py               Block real HTTP/AWS calls during unit tests
    test_models.py            Domain validation tests
    test_itineraries.py       Segment, connection, and round-trip model tests
    test_constraints.py       Deterministic hard-constraint boundaries
    ...
    test_monitoring.py        Cycle orchestration, failure isolation, and completion
    test_scheduling.py        Due times, disabled watches, restarts, and polling
    test_notifications.py     Delivery gating, deduplication, and failure stages
    test_ses.py               Mocked SES request and transport behavior
    test_transport_logging.py Secret exclusion from transport diagnostics
    test_demo.py              Scenario flow, temporary state, and local Live facade
    test_demo_display.py      Current-candidate ID exclusion from display prose
    test_demo_ui.py           Noninteractive Streamlit rendering and explicit actions
    test_demo_deployment.py   Demo-only startup without secrets and Live opt-in gate
```

## Models and SerpApi normalization
...
service below handles delivery. Hosting this unattended monitoring loop remains
deferred; the public Streamlit deployment runs explicit synthetic demo checks.
